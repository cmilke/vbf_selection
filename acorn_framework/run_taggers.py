...
def perform_ML_tagging(event_data_dump):
    ...
    ML_tagger_options = { key:{} for key in tagger_options.keys() }
    ML_tagger_options['2jet']['NN'] = ML_tagger_base('NN', selector_options['all'])
    ML_tagger_options['>=3jet']['3Jdirect'] = direct_3_jet_tagger('3Jdirect')

    # Mass process events with Keras
    for category in event_data_dump.values():
        for jet_ident, available_ML_taggers in ML_tagger_options.items():
            event_list = [ event for event in category.events if jet_ident_counter[jet_ident](event) ]
            for tagger_key, ML_tagger in available_ML_taggers.items():
                print('Mass processing ' + jet_ident + ' with ' + tagger_key)
                ML_tagger.mass_process(tagger_key, event_list)


def tag_events(input_type, args):
    reco_level = '_truth' if args.t else ''
    file_infix = '' if args.infix == None else args.infix+'_'
    ...
    record_name = 'data/output_'+file_infix+args.ntuple+reco_level+'_record_'+input_type+'.p'
    ...
    if not args.T: 
        ...
        perform_ML_tagging(event_data_dump)
    ...
    data_dump_file_name = 'data/output_'+file_infix+args.ntuple+reco_level+'_'+mode+'_'+input_type+'.p'
...
def run():
    ...
    parser.add_argument( "--infix", required = False, default = None, type=str, help = "Load and output to alternate file name",) 
