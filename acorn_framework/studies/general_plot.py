...
for key in ['mjjSL', '2maxpt', 'mjjmax']:
...
for input_key in ['bgd', 'sig']:
    name = 'mjj_'+input_key
    label_list = [ sel+'_'+fil for sel in ['mjjSL', '2maxpt', 'mjjmax'] for fil in ['any', 'central>0.6'] ]
    ...
    _plots[name] = hist2( name, '$M_{jj}$ Correlations Between \'$\Delta$\' 2maxpt and mjjmax for '+_input_titles[input_key],
        50, ( (0,2000), (0,2000) ), xlabel=('$M_{jj}$ between Leading $p_T$ Jets'), ylabel=('$M_{jj}$ between $M_{jj}$-Max Jets') )

    for key0, key1 in itertools.combinations(['mjjSL', '2maxpt', 'mjjmax'],2):
...
def extract_data():
    for input_type in ['bgd', 'sig']:
        ...
        data_dump = pickle.load( open('data/output_cmilke_tag_'+input_type+'.p', 'rb') )
        ...
        for event in JVT_events_with_3_jets:
            for jet in event.jets:
                _plots['correlation_pt_eta_'+input_type].add(jet.vector.eta, jet.vector.pt) 

            Detas, ptsums, mjjs = {}, {}, {}
            for selector_key in ['mjjSL', '2maxpt', 'mjjmax']:
                selector = event.selectors[selector_key]
                jet0 = event.jets[selector.selections[0]]
                jet1 = event.jets[selector.selections[1]]
                Deta = abs(jet0.vector.eta - jet1.vector.eta)
                mjj = ( jet0.vector + jet1.vector ).mass
                Detas[selector_key] = Deta
                ptsums[selector_key] = jet0.vector.pt + jet1.vector.pt
                mjjs[selector_key] = mjj
                for deep_filter_key in ['any', 'central>0.6']:
                    if deep_filter_key in selector.deep_filters:
                        _plots['mjj_'+input_type].add(mjj, selector_key+'_'+deep_filter_key)

                if input_type == 'sig':
                    _plots['correlation_mjj_eta_'+selector_key].add(mjj, Deta)
                    if selector_key == '2maxpt':
                        _plots['leading_eta'].add(jet0.vector.eta, 'any')
                        _plots['leading_eta'].add(jet0.vector.eta, 'any')
                        _plots['leading_eta_delta'].add(Deta, 'any')

            if event.selectors['2maxpt'].selections != event.selectors['mjjmax'].selections:
                _plots['correlation_mjj_2maxpt-vs-mjjmax_Disjoint_'+input_type].add(mjjs['2maxpt'], mjjs['mjjmax'])
